...
async def public_audits_post(request: Request, body: AuditRequest):
    """Public audit endpoint consumed by the GeoReady platform frontend.

    Same pipeline as POST /api/audit (anti-SSRF validation, in-memory cache,
    rate limit, optional Bearer token), plus claim_token / expires_at so the
    frontend's anonymous-claim flow (fetchAuditReport -> mapBackendToFrontend)
    works locally. The native audit JSON is already the shape mapBackendToFrontend
    expects (score_breakdown, robots, llms, schema, meta, content, signals,
    ai_discovery, brand_entity, citability.total_score, report_url, ...).
    """
    import json as _json
    ...
    response = await _run_audit(body.url)
    data = _json.loads(response.body)
    # 1-hour anonymous claim window, matching the in-memory report TTL.
    data["claim_token"] = secrets.token_urlsafe(16)
    data["expires_at"] = datetime.fromtimestamp(time.time() + 3600, tz=timezone.utc).isoformat()
