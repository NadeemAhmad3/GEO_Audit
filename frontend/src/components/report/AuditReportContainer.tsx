...
export default function AuditReportContainer({ reportId }: AuditReportContainerProps) {
  // Demo page initializes with mock data so the island renders fully (SSG).
  const [state, setState] = useState<State>(() =>
    reportId === 'demo'
      ? { status: 'ready', report: mockAuditReport }
      : { status: 'loading' }
  );
  ...
  useEffect(() => {
    ...
    // On a static page query params aren't available at build time — read client-side.
    ...
    // Fallback to the prop reportId (cache hash or encoded URL)
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 md:py-8 space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="p-4 rounded-lg border border-border bg-bg-surface">
          ...
          <div className="mt-1 font-mono text-lg font-semibold tabular-nums">
            <span style={{ color: '#0D9488' }}>{passSignals}</span>
        ...
        <div className="lg:col-span-9 space-y-8">
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[11px] text-text-muted">{report.categories.length} categories</span>
            </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                {criticalCount > 0 && (
                  <span className="text-[11px] font-mono" style={{ color: '#DC2626' }}>{criticalCount} critical</span>
                )}
                {highCount > 0 && (
                  <span className="text-[11px] font-mono" style={{ color: '#0EA5E9' }}>{highCount} high</span>
                )}
